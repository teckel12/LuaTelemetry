...
lcd.drawFilledRectangle(X_CNTR - 10, 55, 20, 8, ERASE)
...
if config[7].v == 1 then
	...
	if data.armed then
		local tmp = 35 - math.floor(varioSpeed * 27 - 0.5)
		if tmp > 35 then
...
lcd.drawText(RIGHT_POS - (data.telemFlags == 0 and 2 or 1), 9, data.satellites % 100, SMLSIZE + RIGHT + data.telemFlags)
...
-- These will have the option of removing
