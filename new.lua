...
local data = loadScript("/SCRIPTS/TELEM/iNAV/data.lua")()

local currentFlightMode = loadScript("/SCRIPTS/TELEM/iNAV/modes.lua")(data.modeId) 
--load it before, not on every flightModes call
...
local function flightModes()
  ...
  if modeIdPrev ~= data.modeId then -- New flight mode
    currentFlightMode = loadScript("/SCRIPTS/TELEM/iNAV/modes.lua")(data.modeId)
    if armed and currentFlightMode.w then
      playFile(WAVPATH .. currentFlightMode.w)
...
local function background()
  ...
  if telemFlags == -1 then
    loadScript("/SCRIPTS/TELEM/iNAV/reset.lua")(data)
  end
